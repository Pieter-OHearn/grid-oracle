...
import { RACES, DRIVERS, RACE_PREDICTIONS, RACE_RESULTS, RACE_ACCURACY, CONSTRUCTOR_COLORS, CONSTRUCTOR_SHORT } from '../data';
...
function getDeltaBorder(delta: number): string {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.05 }} className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        {metrics.map((m, i) => (
          <motion.div key={m.label} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, delay: 0.1 + i * 0.05 }} className="bg-[#0f0f1a] border border-[#1e1e30] rounded-xl p-4 text-center">
            <div className="text-xl mb-1">{m.icon}</div>
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '1.6rem', fontWeight: 800, color: m.color, lineHeight: 1 }}>{m.value}</div>
            <div className="text-[#3a3a52] text-[10px] mt-1 uppercase tracking-wider" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 600 }}>{m.label}</div>
            <div className="text-[#4a4a62] text-[9px] mt-0.5" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{m.sub}</div>
          </motion.div>
        ))}
      </motion.div>
      ...
      <div className="flex items-center gap-4 mb-4">
        <span className="text-[#3a3a52] text-[10px] uppercase tracking-wider" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700 }}>Delta Key:</span>
        {[{ label: 'Exact', color: '#22c55e' }, { label: '±1–2', color: '#eab308' }, { label: '±3–4', color: '#f97316' }, { label: '5+', color: '#ef4444' }].map((k) => (
      ...
      <div className="grid grid-cols-[1fr_44px_1fr] gap-2 mb-2">
        {[{ icon: Target, color: 'text-[#e10600]', label: 'Predicted' }, { icon: Zap, color: 'text-[#22c55e]', label: 'Actual Result' }].map(({ icon: Icon, color, label }) => (
          <div key={label} className="flex items-center gap-2 px-4">
            <Icon size={12} className={color} />
            <span className="text-[10px] uppercase tracking-widest text-[#6b7280]" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700 }}>{label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-1.5">
        {rows.map(({ result, prediction, predictedPos, delta }, idx) => {
          ...
          return (
            <motion.div key={result.driverId} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.15 + idx * 0.022 }} className="grid grid-cols-[1fr_44px_1fr] gap-2 items-center">
              {/* Predicted */}
              <div className="flex items-center gap-3 px-4 py-2.5 rounded-lg border" style={{ background: getDeltaBg(d), borderColor: getDeltaBorder(d) }}>
                <span className="flex-shrink-0 w-8 text-center" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '1rem', fontWeight: 800, color: predictedPos && predictedPos <= 3 ? MEDAL_COLORS[predictedPos - 1] : '#6b7280' }}>
                ...
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <span className="text-white text-sm block truncate" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, letterSpacing: '0.02em' }}>{driver.name}</span>
                    <span className="text-[#4a4a62] text-[10px]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{CONSTRUCTOR_SHORT[driver.constructor]}</span>
                  </div>
                </div>
                {prediction && (
                  <span className="ml-auto text-[10px] flex-shrink-0" style={{ fontFamily: "'JetBrains Mono', monospace", color: '#4a4a62' }}>{prediction.confidence}%</span>
                )}
              </div>

              <DeltaIndicator delta={d} />

              {/* Actual */}
              <div className="flex items-center gap-3 px-4 py-2.5 rounded-lg border" style={{ background: getDeltaBg(d), borderColor: getDeltaBorder(d) }}>
                <span className="flex-shrink-0 w-8 text-center" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '1rem', fontWeight: 800, color: result.position <= 3 ? MEDAL_COLORS[result.position - 1] : '#9090a8' }}>
                ...
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <span className="text-white text-sm block truncate" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, letterSpacing: '0.02em' }}>{driver.name}</span>
                    <span className="text-[#4a4a62] text-[10px]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{result.dnf ? (result.dnfReason ?? 'DNF') : result.time}</span>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 ml-auto flex-shrink-0">
                  {result.fastestLap && (
                    <span className="px-1.5 py-0.5 bg-[#a855f7]/20 text-[#a855f7] border border-[#a855f7]/30 rounded text-[9px] uppercase" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700 }}>FL</span>
                  )}
                  {result.dnf && (
                    <span className="px-1.5 py-0.5 bg-[#ef4444]/15 text-[#ef4444] border border-[#ef4444]/30 rounded text-[9px] uppercase" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700 }}>DNF</span>
                  )}
                </div>
              </div>
            </motion.div>
