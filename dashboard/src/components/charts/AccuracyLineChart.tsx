          ...
          {(['accuracy', 'error'] as const).map((m) => (
            <button
              ...
              className={`px-3 py-1 rounded text-[10px] uppercase tracking-wider transition-all ${
                activeMetric === m ? 'bg-[#e10600] text-white' : 'text-[#6b7280] hover:text-white hover:bg-[#1e1e30]'
              }`}
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={SEASON_CHART_DATA} margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
              ...
              <Legend
                ...
                iconSize={6}
              />
              <Line type="monotone" dataKey="top3" name="Podium Accuracy" stroke="#FFD700" strokeWidth={2} dot={{ fill: '#FFD700', r: 3, strokeWidth: 0 }} activeDot={{ r: 5, fill: '#FFD700' }} />
              <Line type="monotone" dataKey="top10" name="Top 10 Accuracy" stroke="#3b82f6" strokeWidth={2} dot={{ fill: '#3b82f6', r: 3, strokeWidth: 0 }} activeDot={{ r: 5, fill: '#3b82f6' }} />
              <Line type="monotone" dataKey="exactHit" name="Exact Hit Rate" stroke="#22c55e" strokeWidth={2} strokeDasharray="4 2" dot={{ fill: '#22c55e', r: 3, strokeWidth: 0 }} activeDot={{ r: 5, fill: '#22c55e' }} />
            </LineChart>
          </ResponsiveContainer>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={SEASON_CHART_DATA} margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
              ...
              />
              <ReferenceLine y={2.0} stroke="#22c55e" strokeDasharray="4 2" strokeOpacity={0.5} label={{ value: 'Good', fill: '#22c55e', fontSize: 9, fontFamily: "'JetBrains Mono', monospace" }} />
              <ReferenceLine y={3.5} stroke="#ef4444" strokeDasharray="4 2" strokeOpacity={0.5} label={{ value: 'Poor', fill: '#ef4444', fontSize: 9, fontFamily: "'JetBrains Mono', monospace" }} />
              <Line type="monotone" dataKey="mpe" name="Mean Position Error" stroke="#f97316" strokeWidth={2} dot={{ fill: '#f97316', r: 4, strokeWidth: 0 }} activeDot={{ r: 6, fill: '#f97316' }} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
